...
### 배운점
- print(bin(x).count('1')) 로 작성해도 된다. bin은 정수를 이진수로 바꿔주는 함수이다.
<p>while(x):<\p>
<p>x = x&(x-1)</p>
<p>count += 1</p>
<p>이와 같은 비트 단위의 코드를 통해 실행시간을 줄일 수도 있다. 위의 코드는 정수 x의 이진수의 가장 오른쪽 1을 하나씩 제거해주는 코드이다. ex) 1100 & 1011 -> 1000<\p>
