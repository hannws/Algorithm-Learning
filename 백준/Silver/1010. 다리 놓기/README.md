...
### 배운 점
- 계산량을 줄이기 위해 nCm 에서 m = min(n-m, m) 사용하기
- 조합은 무조건 정수이다. 그런데 파이썬에서 나누기는 float 형으로 전환되므로 // 를 사용해서 계산한다.(나머지가 0임)
- n! / (r! * (n-r)!) 일 때 n/r 부터 시작하지 말고 n-r+1 / 1, n-r+2/2 .... 시작해야지 나머지가 남지 않고 계산할 수 있다.
